...
Add a Table of Contents block to your posts and pages. The TOC is a nested list of links to all headings found in the post or page. To use it, simply add a block and search for "WPWingTOC" or just "TOC".
...
* Style WPWingTOC with Gutenberg's native group styling options.
...
WPWingTOC can be found and installed via the Plugin menu within WordPress administration (Plugins -> Add New). Alternatively, it can be downloaded from WordPress.org and installed manually...

In Gutenberg, add a block and search for "WPWingTOC" or just "TOC". Please save your content before you use the block.
...
Hide the headline in the sidebar options of WPWingTOC and add your own heading.

= How do I add WPWingTOC to all articles automatically?  =

I don’t see an easy solution at the moment. WPWingTOC is only a block that can be placed in your post. If there would be a plugin that adds blocks to every post then this would be the solution. I think this should be another separate plug-in to keep the code of WPWingTOC clean and … well, simple. Maybe someone knows of a plug-in that adds blocks automatically to all posts with some parameters and settings? What about site editing in WordPress? I think the core team is working on something like that. I will keep this post open. If I have gained more knowledge on how to solve this I will add this feature.

= How do I add a background color to WPWingTOC using Gutenberg groups? =

Select the block and select "group" in the context menu. Apply "background color", "link color" and "text color" to this group. WPWingTOC will inherit these styles. You would like to use this styled WPWingTOC group next time you write a post? Convert it to a reusable block.
...
If you really want to hide a single heading from the table of contents then add the CSS class "wpwing-toc-hidden" to a heading block. But first of all, think about the reason you want to hide a specific heading. Maybe you want to remove all headins of a specific depth level. Then there is an option for that in the blocks options in Gutenberg. If you think this heading should not be part of the toc maybe it is not needed in the post itself?

== Screenshots ==
1. WPWingTOC block in Gutenberg editor.
2. WPWingTOC in the post.
3. Simple but powerful. Customize each TOC as you like.
4. Control the maximum depth of the headings.
5. Style WPWingTOC with Gutenbergs native group styling options.

== Credits ==

WPWingTOC is developed on GitHub: https://github.com/wpwing-dev/wpwing-table-of-contens-block